...
static int quad_enable(const struct device *dev, const bool enable)
{
	...
	/* write enable */
	ret = spi_nor_cmd_write(dev, spi_cfg, spi_cfg, spi_cfg, SPI_NOR_CMD_WREN);
	do {
	...
	/* Write disable */
	ret = spi_nor_cmd_write(dev, spi_cfg, spi_cfg, spi_cfg, SPI_NOR_CMD_WRDI);
	do {
		ret = spi_nor_cmd_read(dev, spi_cfg, SPI_NOR_CMD_RDSR, &reg, sizeof(reg));
		k_sleep(K_MSEC(10));
	} while ((reg & SPI_NOR_WEL_BIT) == SPI_NOR_WEL_BIT);
...
#if CONFIG_SPI_TEST_PLL_FREQ_SELECT
	source_clk = MHZ(48); /* pll frequency */
#else
	source_clk = KHZ(9200); /* ec frequency */
#endif /* CONFIG_SPI_TEST_PLL_FREQ_SELECT */
